...
> [!NOTE] `adapter-cloudflare-workers` は非推奨となり、代わりに[`adapter-cloudflare`](adapter-cloudflare) をお使いください。Cloudflare Workers Sites が非推奨となるため、[静的アセット](https://developers.cloudflare.com/workers/static-assets/) を使用して Cloudflare Workers にデプロイする場合は、`adapter-cloudflare` の使用をおすすめします。
